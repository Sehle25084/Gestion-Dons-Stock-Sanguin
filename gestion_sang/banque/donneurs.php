...
if (isset($_POST['confirmer'])) {
    ...
    $pdo->prepare("UPDATE donneur SET id_groupe = ? WHERE id_donneur = ?")->execute([$id_groupe, $id_donneur]);
    $success = "Groupe sanguin confirmé avec succès !";
}

// Récupération des donneurs non vérifiés
$non_verifies = $pdo->query("SELECT * FROM donneur WHERE id_groupe IS NULL ORDER BY id_donneur DESC")->fetchAll();

// Récupération de tous les donneurs vérifiés
$tous = $pdo->query("SELECT * FROM donneur WHERE id_groupe IS NOT NULL ORDER BY id_donneur DESC")->fetchAll();
