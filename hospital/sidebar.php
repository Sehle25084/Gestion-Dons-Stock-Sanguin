...
if (isset($_SESSION['id_hopital'])) {
    $stmt_notif = $pdo->prepare("SELECT COUNT(*) FROM notification_hopital WHERE id_hopital = ? AND lu = 0");
    $stmt_notif->execute([$_SESSION['id_hopital']]);
    $nb_notifs_non_lues = $stmt_notif->fetchColumn();
}
?>
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .btn-new-demande:hover { background: #6B0000; transform: translateY(-1px); }
    .btn-new-demande svg { width: 16px; height: 16px; flex-shrink: 0; }
    ...
    .stat-card.c5:hover { border-color: #DC2626; }
    .stat-card.c6:hover { border-color: #7C3AED; }
    ...
    .ic-pur  { background: #F5F3FF; border-color: #C4B5FD; }
    .ic-pnk  { background: #FDF2F8; border-color: #F9A8D4; }
    ...
    .badge-annulee     { background: #F3F4F6; color: #6B7280; border: 2px solid #D1D5DB; }
    ...
    /* ══ STOCK BAR ══ */
    .stock-bar { width: 100%; height: 8px; background: #F3F4F6; border-radius: 10px; margin-top: 6px; }
    .stock-bar-fill { height: 100%; background: #8B0000; border-radius: 10px; transition: width 0.4s ease; }
...
<aside class="sidebar">
    <div class="sidebar-logo">
        <svg width="210" height="70" viewBox="0 0 210 70" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="50" y="50" font-family="Arial, sans-serif" font-size="11" font-weight="500" fill="#444444"><?php echo htmlspecialchars($_SESSION['nom_hopital'] ?? ''); ?></text>
        </svg>
    </div>

    <a href="demandes.php?action=nouvelle" class="btn-new-demande">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            <line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>
        </svg>
        Envoyer une nouvelle demande de sang
    </a>
    ...
    <div class="sidebar-footer">
        <div class="hospital-info">
            <div class="avatar"><?php echo strtoupper(substr($_SESSION['prenom_responsable'] ?? 'X', 0, 1) . substr($_SESSION['nom_responsable'] ?? 'X', 0, 1)); ?></div>
            <div>
                <div class="hospital-name"><?php echo htmlspecialchars(($_SESSION['prenom_responsable'] ?? '') . ' ' . ($_SESSION['nom_responsable'] ?? '')); ?></div>
                <div class="hospital-type"><?php echo htmlspecialchars($_SESSION['poste_responsable'] ?? ''); ?></div>
            </div>
        </div>
